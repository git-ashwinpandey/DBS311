...
SELECT 
    ...
    country_id
FROM locations
WHERE trim(upper(city)) LIKE '%' || trim(upper('&CityName')) || '%';
...
SELECT
    ...
    salary,
    department_name,
    ROUND(salary * 1.04, 0) AS GoodSalary,
    (ROUND(salary * 1.04, 0) - salary) * 12 AS AnnualPayIncrease
FROM 
    employees e
...
SELECT
    last_name, 
    hire_date,
    TRUNC(((sysdate-hire_date)/365.25),0) AS YearsWorked -- i do not like 365
FROM employees
...
SELECT  last_name, 
        hire_date,
        TRUNC(months_between(sysdate,hire_date)/12,0) AS YearsWorked
FROM employees
WHERE hire_date < to_date('20140101','yyyymmdd')
ORDER BY YearsWorked;
...
SELECT
    ...
    to_char ( 
        Next_Day ( 
            ...
            ) , 'fmDAY, MONTH "the" Ddspth "of year" yyyy' ) AS ReviewDay
FROM employees
